    ...
    */
    if (firstChoice && secondChoice) {
      ...
      if (firstChoice.card === secondChoice.card) {
        console.log("cards: ", firstChoice.card, " and ", secondChoice.card, " match!")
        ...
        console.log("cards: ", firstChoice.card, " and ", secondChoice.card, " DO NOT match!")
  ...
  return (
    <>
      <div className="game-body">
        <Title style={{ color: "#FFD5FF" }}>{"Memory Game"}</Title>

        {/* Announce Winner when the game is over */}
        {gameOver ? <Title style={{ color: "#FF0000" }}>{`${winner}`}</Title> : null}
        ...
        {showPlayerPrompt ?
          <>
            ...
            <p></p>
            <StartMenu numberOfPlayers={selectNumberOfPlayers} player1Name={setP1Name} player2Name={setP2Name} multiplayer={twoPlayer} beginGame={beginGame} />
          </> : null}
        
        {/*-------------------------------------------------------*/}
        {/* Display score once the player/s have chosen number of players */}
        {/*-------------------------------------------------------*/}
        {!showPlayerPrompt ? <div style={{ width:"1000px"}}>
          <Row>
            <Col span={8} offset={8}>{`Total Matches: ${totalMatches}/${deck.length/2}`}</Col>
          </Row>
          <Row>
            <Col span={8}>{`${player1Name}'s Score: ${player1Score}`}</Col>
            {twoPlayer ? <Col span={8} offset={8}>
        ...
        <Card>
          <div className="card-grid">
            {deck.map((card) => (
              <div className="card" key={card.key} data-testid={card.key}>
                <SingleCard key={card.key} card={card} selectCard={selectCard} reveal={card === firstChoice || card === secondChoice || card.match} disableCard={disableCard} />
              </div>
            ))}
          </div>
        </Card>
